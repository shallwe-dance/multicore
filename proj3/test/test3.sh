#!/bin/bash
# recovery_test.sh
rm -f coordinator_*.log participant_*.log
...
./coordinator --id 0 --prog 0x20000000 --conf participants.conf --fail-after-prepare > $LOG_DIR/coordinator.log 2>&1 &
COORD=$!
sleep 3 # allow transaction to start and crash
# Kill coordinator process if it hasn’t exited
